...
export interface Proposal {
  ...
  /**
   * Self-contained HTML body (adj-200). Additive to `description`, which stays
   * the required markdown source for list previews, search, and scoring. Absent
   * on legacy proposals created before the public-HTML epic.
   */
  html?: string;
  /** Whether the proposal is published to a no-API-key public page. */
  isPublic?: boolean;
  /** Unguessable public handle; present once the proposal has been published. */
  shareToken?: string;
