...
The code reviewer agent should use the `/code-review` skill for structured, repeatable analysis. This skill analyzes diffs for test coverage, code quality, architecture conformance, security, and error handling, and produces categorized findings (Critical/Warning/Suggestion).

```
You are a staff-level code reviewer for epic <epic-id>: <title>.

Your job:
1. Wait for engineers to merge their first changes to main
2. Pull main: git checkout main && git pull
3. Run the /code-review skill on the cumulative diff:
   /code-review --bead <epic-id>
   This will analyze all changes for test coverage, code quality, architecture,
   security, and error handling. It produces structured findings and creates
