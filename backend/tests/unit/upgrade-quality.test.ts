...
describe("syncQualityFiles", () => {
  ...
  it("should create missing quality files", () => {
    ...
    // skipIfExists files should be skipped
  ...
  it("should skip up-to-date files", () => {
    // Pre-populate all non-skipIfExists files with template content
    const nonSkipped = QUALITY_FILES.filter((qf) => !qf.skipIfExists);
    for (const qf of nonSkipped) {
      const fullPath = join(tempDir, qf.destPath);
  ...
  it("should update outdated files with line count diff", () => {
    ...
    for (const qf of nonSkipped) {
      ...
      expect(result, `Missing result for ${qf.destPath}`).toBeDefined();
      expect(result!.status).toBe("created");
      // Message should include line count change: "updated (4 → N lines)"
